					...
					if($stmt->rowCount()>0){
						...
						while($row=$stmt->fetch(PDO::FETCH_ASSOC)){
							...
							if($QtypeCode==1){
									...
									echo "</td>\n";
									echo '<td><label><h4 id="'.$nextT.'">'.$HeaderCaption.'<input type=\'hidden\' id=\'obj_Q-'.$id.'\'><input type=\'hidden\' id=\'obj_Idx-'.$id.'\'></h4></label><h4 id="CT-'.$i.'"></h4></td>';
									...
									if(intval($uniform)===1){
										...
										while($rowq=$stmtQC->fetch(PDO::FETCH_ASSOC)){
											...
											echo "<tr>\n";
												echo "<td width='50px'>&nbsp;</td>\n";
												echo "<td width='50px' align='center'>\n";
												echo '<input type="radio"  name="obj_Q-'.$id.'" class="btn-check"  onchange=\'setQuestValueIndex("obj_Q-'.$id.'","obj_Idx-'.$id.'",'.$indexNo.','.$weight.',"'."T-".$id.'")\'>&nbsp;';
												echo "</td>\n";
												echo "<td>\n";
													echo "<h5>".$caption."</h5>";
												echo "</td>\n";
...
<script>
	...
	function getDepress2Q(){
		var url="<?=$rootPath?>/burnout/getDepress2Q.php?userCode="+$("#obj_userCode").val();
		...
		$("#btnSaveSurvey").click(function(){
			...
			if(!isSurvey()){
						...
						swal.fire({
							...
							if(getDepress2Q()>0){
								var url="<?=$rootPath?>/tprojectgroup/genDepress9Q.php?projectGroupId=8&userCode="+$("#obj_userCode").val()+"&userType=<?=$userType?>";
								//console.log(url);
								$("#dvContent").load(url);
							}else
							{
