					...
					if($stmt->rowCount()>0){
						...
						while($row=$stmt->fetch(PDO::FETCH_ASSOC)){
							...
							if($QtypeCode==1){
									...
									echo "</td>\n";
									echo '<td><label><h4>'.$HeaderCaption.'<input type=\'hidden\' id=\'obj_Q-'.$id.'\'></h4></label></td>';
									...
									foreach ($objLabels as $row) {
										...
										echo '<input type="radio" class="btn-check" name="obj_Q-'.$id.'"  onchange=\'setQuestValue("obj_Q-'.$id.'",'.$i.')\'>&nbsp;';
