...
<?php
	...
	$url=$cnf->restURL."data/getSurveyAnswer.php?surveyDate=".$surveyDate."&userType=".$userType;
	...
	if(!isset($data["message"])){
		...
		foreach ($data as $row) {
			...
			if(intval($row["isFollow"])===0){
				...
				$strV="<table width='100%'><tr><td valign='center'><a class='btn btn-success'><i class=\"fa fa-handshake-o\" aria-hidden=\"true\"></i>&nbsp;ติดตามแล้ว</a></td></td></table>\n";
				echo "<td>".$strV."</td>\n";
				
			}

			//echo "<td>".intval($row["isFollow"])."</td>\n";
			
			array_push($bArr, $row["burnoutDef"]);
			$strB="<h5>".$row["burnoutDef"]["description"]."</h5>\n";
			$strB.="<div id='dB".$i."' style=\"text-align:left\"></div>";
			echo "<td width=\"12%\" >".$strB."</td>\n";
