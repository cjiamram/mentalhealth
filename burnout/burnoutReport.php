	<?php
		...
		$cnf=new Config();
		$rootPath=$cnf->path;
		$emotionalLackValue=$obj->getEmotionalLack($userCode);
		$personalLackValue=$obj->getPersonalLack($userCode);
		$successValue=$obj->getSuccess($userCode);
...
.right-gap {
  margin-right: auto;
	...
	<div class="box box-success">
		...
		?>
	<tr><td colspan="2">
	<button id="obj_questionair" class="btn btn-primary pull-left"><i class="fa fa-check"  aria-hidden="true"></i> ทำแบบทดสอบอีกครั้ง</bunton>&nbsp;
	<button id="obj_logout" class="btn btn-warning pull-left"><i class="fa fa-sign-out"  aria-hidden="true"></i> ออกจากระบบ</bunton>
	...
	<table class="table table-bordered  table-hover">
		<tr>
			<td  align="center">
				<h3 style='color:blue'>การแบ่งระดับความเหนื่อยหน่าย (Maslach & Jackson, 1986)</h3>
			</td>
		</tr>
		<tr><td>
			<div id="dvT" style="display:block">
			<div class='col-sm-6'>
				<h4>ความเหนื่อยหน่าย</h4>
				<a id="obj_burnoutdetail" class="btn btn-danger"><i class="fa fa-info-circle"  aria-hidden="true"></i><label>&nbsp;คำแนะนำ</label></a>
				<a id="obj_burnoutT" class="btn btn-success"><i class="fa fa-info-circle" aria-hidden="true"></i><label>&nbsp;คำตอบ</label></a>

			</div>
			<div class='col-sm-6'>
				<iframe id="dvBurnoutTotal" frameborder='0' scrolling="no" width='100%' height='200px' ></iframe>
			</div>
			
			</div>
			<div id="dvD" style="display:none"> 
			<div class='col-sm-6'>
				<h4>ความอ่อนล้าทางอารมณ์</h4>
			</div>
			<div class='col-sm-6'>
				<iframe id="dvElack" frameborder='0' scrolling="no" width='100%' height='200px' ></iframe>
			</div>
			<div class='col-sm-6'>
				<h4>การลดความเป็นบุคคล</h4> 
			</div>
			<div class='col-sm-6'>
				<iframe id="dvPlack" frameborder='0' scrolling="no" width='100%' height='200px' ></iframe>
			</div>
			<div class='col-sm-6'><h4>ความสำเร็จส่วนบุคคล</h4> 
			</div>
			<div class='col-sm-6'>
				<iframe id="dvSuccess" frameborder='0' scrolling="no" width='100%' height='200px' ></iframe>
			</div>
			</div>
	...
	<div class="box box-primary">
		<table class="table table-bordered  table-hover">
			...
			<tr>
		<td>
			<div class='col-sm-6'><h4>ความเข้มแข็งทางใจ</h4>
	...
	<div class="box box-danger" id="dv2Q" style="display:block">
		<table class="table table-bordered  table-hover">
			...
			<tr>
		<td>
			<div class='col-sm-6'><h4>ภาวะซึมเศร้า (2Q)</h4> 
			</div>
			<div class='col-sm-6'>
				<?php
					if($depress2QValue>0){
	...
	<div class="box box-danger" id="dv8Q" style="display:none"  >
		<table class="table table-bordered  table-hover">
			...
			<tr>
		<td>
			<div class='col-sm-6'><h4>ภาวะการฆ่าตัวตาย </h4>
	...
	<div>
		<button id="obj_sendReport" class="btn btn-success pull-left"><i class="fa fa-envelope"  aria-hidden="true"></i> ส่งผลลัพท์เพื่อรายงาน</bunton>
	</div>
	...
	<script>
		...
		function setEvaluation(){
			...
			var burnouteRate='<?=$totalBurnOut?>';
			var burnouteGrade="";

			if(burnouteRate>=0&&burnouteRate<=53){
				burnouteGrade="01";
			} else
			if(burnouteRate>53&&burnouteRate<=78){
				burnouteGrade="02";
			}else
			if(burnouteRate>78){
				burnouteGrade="03";
			...
			var url="";
			if(data.flag==true)
				url="<?=$rootPath?>/tevaluate/update.php";
			else
				url="<?=$rootPath?>/tevaluate/create.php";
			var data1=executeData(url,jsonObj,false);
		}
		
		function setTotalBurnout(){
			var url="<?=$rootPath?>/burnout/totalBurnOut.php?totalBurnOut=<?=$totalBurnOut?>";
			$("#dvBurnoutTotal").attr('src', url);
		}

		function setElack(){
			var url="<?=$rootPath?>/burnout/emotionalLack.php?emotionalLackValue=<?=$emotionalLackValue?>";
			$("#dvElack").attr('src', url);
		}

		function setPlack(){
			var url="<?=$rootPath?>/burnout/personalLack.php?personalLackValue=<?=$personalLackValue?>";
			$("#dvPlack").attr('src', url);
		}

		function setSuccess(){
			var url="<?=$rootPath?>/burnout/success.php?successValue=<?=$successValue?>";
			$("#dvSuccess").attr('src', url);
		}
		...
		function setBurnout(){
			var totalBurnOut='<?=$totalBurnOut?>';
			var url="<?=$rootPath?>/tsuggestion/getData.php?projectId=10&keyIndex=GREEN";
			 ...
			 if(totalBurnOut>53&&totalBurnOut<=78){
			 	var url="<?=$rootPath?>/tsuggestion/getData.php?projectId=10&keyIndex=YELLOW";

			 }else
			 if(totalBurnOut>78){
			 	var url="<?=$rootPath?>/tsuggestion/getData.php?projectId=10&keyIndex=RED";
		...
		function sendMailByLevel(sendOrder){
			var url="<?=$rootPath?>/sendmailT.php?userCode=<?=$userCode?>&userType=2&sendOrder="+sendOrder;
			console.log(url);	
		...
		function sendMailByFaculty(faculty){
			var url="<?=$rootPath?>/sendmailByFaculty.php?userCode=<?=$userCode?>&userType=2&faculty="+faculty;
				var data=executeGet(url);
				if(data.message==true){
					swal.fire({
				      title: "การส่งรายงานสำเร็จแล้ว",
		...
		function setEmailValidate(){
			...
			if(facultyCode==="020000"){
				sendMailByFaculty(facultyCode);
			}

			
			if((depress9QValue>=0&&depress9QValue<=7)&&(depress9Q_1Value)==0){
				orderSend=1;
			}else
			if(depress9QValue>7){
				orderSend=2;
				sendMailByLevel(orderSend);
			...
			var suicideValue='<?=$suicideValue?>';
			if(suicideValue>=0&&suicideValue<=7){
				orderSend=1;
			}else{
				orderSend=2;
				sendMailByLevel(orderSend);
		...
		function setBurnoutAnswer(){
			
			var url="<?=$rootPath?>/data/displayAnswerBurnout.php?userCode=<?=$userCode?>";
			$("#tbRender").load(url);
		...
		function saveIssend(){
			var url="<?=$rootPath?>/tissend/create.php";
		...
		$(document).ready(function(){
			...
			$("#obj_t").click(function(){
				if($("#obj_toggle").val()==0){
					$("#dvBMsg").text("แสดงรายละเอียด");
					$("#obj_toggle").val(1);
					$("#dvD").attr('style','display:block');
					setElack();
					setPlack();
					setSuccess();
					...
					$("#obj_toggle").val(0);
					$("#dvBMsg").text("ซ่อนรายละเอียด");

					$("#dvD").attr('style','display:none');
				}
			});
		});	
